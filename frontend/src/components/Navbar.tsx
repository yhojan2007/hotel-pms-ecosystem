...
import { Hotel, Radio, Sparkles, RefreshCw } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ isConnected, onOpenSimulator, onRefresh }) => {
  return (
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
              EcoPMS Cloud <span className="text-xs px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400 border border-indigo-500/30">Live Demo</span>
            </h1>
            <p className="text-xs text-gray-400">PMS en Tiempo Real + Agente Autónomo WhatsApp</p>
          </div>
        </div>

        {/* Estado WebSocket & Acciones */}
