"""Orquestador del agente: Claude + tools, o simulador de demo sin API key."""
...
async def process_incoming_message(
    ...
    """Procesa texto (o audio ya transcrito), ejecuta tools y responde por WhatsApp.

    Si hay ``ANTHROPIC_API_KEY`` válida usa Claude con tool calling.
    Si no, un fallback por palabras clave que igual invoca las tools reales
    (fechas fijas 2026-08-01 / 2026-08-05 para la demo).
    """
    ...
    if "reservar" in msg_lower or "reserva" in msg_lower or "confirmar" in msg_lower:
        ...
        if "error" in tool_res:
            ...
            pay_res = await execute_tool_call(
        ...
        avail_res = await execute_tool_call(
            "check_room_availability",
            {
                "fecha_checkin": "2026-08-01",
                "fecha_checkout": "2026-08-05",
